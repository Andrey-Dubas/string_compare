...
int main(int argc, char *argv[]){
    ...
    has_static<int>::fuh();
    std::set<std::string> arg_container;

    std::for_each(argv + 1, argv + argc ,
        [&arg_container](const char* arg){
            arg_container.insert(arg);
        });

    if (arg_container.find("-help") != arg_container.end()){
        std::cout
            << "strcmp file [-u]\n"
                << "file you puts your string to compare\nin pairs\n"
            << "-u - consider ist content in unicode\n";
        return 0;
    }
    ...
    bool isUnicode = !(arg_container.find("-u") == arg_container.end());

    if(isUnicode == false){
        std::string filename = argv[1];
        ComparativeHolderCollection<char> container = getComparableStrings<char>(filename);

        for(ComparativeHolderCollection<char>::iterator it = container.begin(); it != container.end(); ++it){

            float similar = 2 * compareFunction(it->first, it->second) / (float)(it->first.length() + it->second.length());
            std::cout << "similarity between \"" << it->first.c_str() << "\" and \"" <<  it->second.c_str() << "\" is: " << similar << std::endl;
        }
    }
    else{
        std::string filename = argv[1];
        ...
        for(ComparativeHolderCollection<char>::iterator it = container.begin(); it != container.end(); ++it){
            float similar = 2 * compareFunction(it->first, it->second) / (float)(it->first.length() + it->second.length());
            std::wcout << "similarity between \"" << it->first.c_str() << "\" and \"" <<  it->second.c_str() << "\" is: " << similar << std::endl;
        }
    }
